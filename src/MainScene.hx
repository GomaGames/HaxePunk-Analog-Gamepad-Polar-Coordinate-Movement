...
class MainScene extends Scene
{
  ...
  private var angular_velocity:Float;
  ...
  public override function begin()
  {
     ...
     */
    shape_image = Image.createRect( 50, 50, 0x3366FF );
  ...
  {
    // rotation
    shape_image.angle += angular_velocity;

    /*
      Get the angle of the analog stick in radians
      Math.atan2(y2 - y1, x2 - x1) * (-180 / Math.PI)
     */
    var angle = HXP.angle(0, 0, my_shape_x_velocity, my_shape_y_velocity);

    /*
      Get the speed based on the analog pressure or "distance" of the joystick
      Math.sqrt((x2 - x1) * (x2 - x1) + (y2 - y1) * (y2 - y1));
     */
    var speed = HXP.distance(0, 0, my_shape_x_velocity, my_shape_y_velocity);

    /*
      Move the shape
      http://haxepunk.com/documentation/api/com/haxepunk/Entity.html#moveAtAngle
     */
    my_shape.moveAtAngle(angle, speed * my_shape_speed);
  }
  ...
  private inline function handleInput()
  {
    ...
    if ( player_1_gamepad.getAxis( XBOX_GAMEPAD.LEFT_ANALOGUE_X ) != 0 ){
      ...
      my_shape_x_velocity = player_1_gamepad.getAxis( XBOX_GAMEPAD.LEFT_ANALOGUE_X ) ;
    ...
    if ( player_1_gamepad.getAxis( XBOX_GAMEPAD.LEFT_ANALOGUE_Y ) != 0 ){
      ...
      my_shape_y_velocity = player_1_gamepad.getAxis( XBOX_GAMEPAD.LEFT_ANALOGUE_Y );
    ...
    if ( player_1_gamepad.getAxis( XBOX_GAMEPAD.RIGHT_ANALOGUE_X ) != 0 ){
      ...
      // rotate shape
      angular_velocity = my_shape_rotate_speed * player_1_gamepad.getAxis( XBOX_GAMEPAD.RIGHT_ANALOGUE_X );

    }else{

      // stop rotating
      angular_velocity *= .9; // set to = 0; to stop hard
